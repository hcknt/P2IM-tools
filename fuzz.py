...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume", dest="resume",
        action='store_true', help="resume previous fuzzing session")
    parser.add_argument("--reset", dest="reset",
        action='store_true', help="reset fuzzing environment (keep only inputs, fuzz.cfg, firmware elf)")
    ...
    # Reset fuzzing environment if requested
    if args.reset:
        color_print("Resetting fuzzing environment...", "yellow")
        
        # List of files/directories to keep
        keep_files = {'inputs', 'fuzz.cfg', os.path.basename(cfg.img)}
        
        # Remove all files and directories except the ones we want to keep
        for item in os.listdir('.'):
            if item not in keep_files:
                item_path = os.path.join('.', item)
                if os.path.isdir(item_path):
                    color_print("Removing directory: %s" % item, "red")
                    shutil.rmtree(item_path)
                else:
                    color_print("Removing file: %s" % item, "red")
                    os.remove(item_path)
        
        color_print("Reset completed! Kept: inputs/, fuzz.cfg, firmware elf", "green")
        print("")
    ...
    if args.resume and os.path.exists(cfg.afl_output):
        color_print("Resuming previous fuzzing session", "yellow")
        cmd_afl = [cfg.afl_bin, "-i-", "-o", cfg.afl_output, 
